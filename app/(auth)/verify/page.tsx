      ...
      (user: FirebaseUser | null) => {
        ...
        if (user) {
          ...
          if (user.emailVerified) {
            ...
            setMessage(
              "Please verify your email. If you received a code, enter it below, or request a new verification email."
            );
          }
        } else {
          ...
          if (!emailFromQuery) {
            setError(
              "No user session found. Please log in or ensure you have an email in the link to verify."
            );
          } else {
            setMessage(
              `Attempting to verify for ${emailFromQuery}. If you received a code, enter it below.`
            );
          }
        }
  ...
  const handleResendOTP = async () => {
    ...
    try {
      if (auth.currentUser && !auth.currentUser.emailVerified) {
        ...
        setError(
          "Please ensure you are logged in with the email you want to verify, or check the email sent during signup."
        );
      }
    } catch (error: unknown) {
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-2">
            ...
            <span className="font-medium">
              {currentUserEmail || emailFromQuery || "your email"}
            </span>
            .
          </p>
